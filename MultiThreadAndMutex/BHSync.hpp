 ...
 *                         借鉴了 MacOS/iOS GCD 的设计思想
 *
 * This file provides a set of synchronization tools based on std::atomic:
 *   - SmartLock         : high-performance spin lock with yield/sleep fallback
 *   - RWSmartLock       : high-performance read-write spin lock
 *   - RecursiveSmartLock: recursive lock that does not self-block
 *   - AtomicSemaphore   : atomic semaphore for multi-thread synchronization
 *   - BHGCDController   : queue-based task scheduler with barrier support
 *
 * 功能简述 / Features:
 ...
 * 6. barrier 可用于读写互斥场景，简化读写互斥操作
 * 7. group dispatch 可简化并行任务依赖关系，在多个任务完成后执行后续操作
...
inline int randomInt(int min = 0, int max = 1000000) {
...
protected:
    std::atomic<bool> flag;  // 原子标志位 / Atomic flag
...
public:
    SpinLockBase(bool needSleep = true, int spin = 5, int yield = 8, int sleep_us = 3)
        : flag(false), _maxSpinCount(spin), _maxYieldCount(yield), _sleepMicros(sleep_us), _needSleep(needSleep) {}

    void unlock() { flag.store(false, std::memory_order_release); }  // 解锁 / Unlock
};
...
public:
    ...
    void enqueueGroup(int groupId,const std::function<void()>& func) {
        ...
        if (_activeGroups.find(groupId) != _activeGroups.end()) {
    ...
    void setCallbackForGroup(int groupId,
        ...
        _groupOptLock.lock();
        if (_existGroups.find(groupId) != _existGroups.end()) {
...
private:
    struct TaskItem {
        TaskItem() : groupID(-1), isBarrier(false) {}
        TaskItem(int gid, const std::function<void()>& fn, bool barrier)
            : groupID(gid), func(fn), isBarrier(barrier) {}
    ...
    inline void excueteGroupTask(TaskItem &task) {
        ...
        while (_activeGroups.find(groupId) == _activeGroups.end()) {
            if (yieldCount > 15) {
        ...
        if (_groupActiveTaskCount[groupId] == 0) {
            // 当前组的任务执行完毕的情况下。
            // 执行当前组的回调
            if (_groupIdCallback.find(groupId) != _groupIdCallback.end()) {
